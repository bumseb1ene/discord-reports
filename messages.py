...
async def unitreportembed(player_additional_data, user_lang, unit_name, roles, team, player):
    ...
    embed.add_field(name=get_translation(user_lang, "total_playtime"),
                    value=f"{total_playtime_hours:.2f} " + get_translation(user_lang, "hours"),
                    inline=True)
...
class Reportview(discord.ui.View):
    ...
    async def add_buttons(self, user_lang, reported_player_name, player_id, self_report=False, player_found=True):
        if not self_report:
            author_name = get_author_name() or "Unbekannter Melder"
