...
class MyBot(commands.Bot):
    ...
    async def player_not_found(self, message):
        ...
        Statt eines statischen Texts wird nun die KI-Antwort verwendet.
        """
        ...
        not_found_text = await generate_player_not_found_text(self.ai_client, author_name, self.user_lang)
        ...
        # Embed erstellen (zum Beispiel via vorhandener player_not_found_embed-Funktion)
        embed = await player_not_found_embed(author_player_id, author_name, self.user_lang)
        
        # Optional: KI-Antwort in das Embed integrieren (als zusätzliches Feld)
        embed.add_field(name=get_translation(self.user_lang, "ai_generated_response"), value=not_found_text, inline=False)
